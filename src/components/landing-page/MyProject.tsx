    ...
    return (
        <section className=" p-[100px_300px_50px] 1560:p-[60px_30px_50px] flex flex-col items-center justify-center ">
            <div className="w-full flex items-center justify-center flex-col">
                <h2 className="text-[#ffffff] relative z-[3]  text-[48px] font-[600] 795:text-[26px]">Portfolio</h2>
                <p className="text-center relative z-[3]  font-[400] text-[#ffffff] text-[18px]  795:text-[14px]">
                    even more projects can be seen in my GitHub
                </p>
            </div>

            <div className='w-full mt-[70px] grid grid-cols-[repeat(auto-fit,_minmax(280px,_424px))] gap-[32px] justify-center relative z-[3]  drop-shadow-[0_10px_10px_rgba(43,56,76,0.1)]'>
                {PortfolioData.map(item =>
                    <Link style={{position: "relative", zIndex: "3"}}   href={item.link}  target='blank' key={item.id} className='flex flex-col h-full justify-between rounded-[22px] cursor-pointer duration-200 group hover:scale-[1.02] '>
            ...
            <div className='w-full flex justify-center mt-[50px]'>
                <Link href={"https://github.com/scDjanGo"} target='blank' className='bg-[#A53DFF] relative z-[3] rounded-[4px] text-[#ffffff] p-[12px_24px] border-[1px] border-[#A53DFF] duration-200 hover:text-[#A53DFF] hover:bg-[#ffffff]'>More Project</Link>
            </div>
