...
//                 <h3 className="text-[48px] font-[600] text-[#333333] 795:text-[26px]">What I do?</h3>

//                 <div className="flex flex-col gap-[16px]">
//                     <p className="text-[#87909D] text-[18px] font-[400] 795:text-[12px]">
...
//                         className={`relative p-[32px] 795:p-[16px] flex flex-col gap-[16px] rounded-[12px] bg-[#ffffff] hover:drop-shadow-[0_10px_10px_rgba(43,56,76,0.1)] cursor-pointer about-me-child ${item.id}  ${hoveredId === item.id && " !rounded-[0]"} `}
//                     >
//                         <h3 className="text-[#132238] text-[24px] font-[600] 795:text-[18px]">{item.title}</h3>
//                         <p className="text-[#424E60] text-[16px] font-[400] 795:text-[12px]">{item.description}</p>
...
export default function AboutMe() {
    ...
    return (
        <div
            id="aboutMe"
            className="flex 795:flex-col items-center gap-[25px] justify-between bg-[#F0F1F3] p-[150px_300px] 1560:p-[100px_30px] overflow-x-hidden"
        >
            <div className="max-w-[530px] flex flex-col gap-[24px] 795:items-center">
                <h3 className="text-[48px] font-[600] text-[#333333] 795:text-[26px]">What I do?</h3>
                ...
                <div className="flex flex-col gap-[16px]">
                    <p className="text-[#87909D] text-[18px] font-[400] 795:text-[12px]">
                        As a Frontend developer, I specialize in building user interfaces for web applications that
                        prioritize usability and intuitive interactions.{" "}
                    </p>
                    <p className="text-[#87909D] text-[18px] font-[400] 795:text-[12px]">
                        I collaborate closely with designers and backend developers to deliver projects from concept to
                        final product.
                    </p>
                </div>
            </div>
            ...
            <div className="flex flex-col gap-[24px] max-w-[650px]">
                {AboutMe.map((item, index) => (
                    <motion.div
                        ...
                        onMouseEnter={() => handleMouseEnter(item.id)}
                        onMouseLeave={handleMouseLeave}
                        className={`relative p-[32px] 795:p-[16px] flex flex-col gap-[16px] rounded-[12px] bg-[#ffffff] hover:drop-shadow-[0_10px_10px_rgba(43,56,76,0.1)] cursor-pointer about-me-child ${
                            item.id
                        }  ${hoveredId === item.id && " !rounded-[0]"}`}
                        initial={{
                            x: index % 2 === 0 ? "-100vw" : "100vw", // Первый элемент слева, второй справа
                            opacity: 0,
                        }}
                        animate={{
                            x: 0,
                        ...
                        transition={{
                            duration: 0.6,
                            delay: index * 0.3, // Задержка для последовательной анимации
                            ease: "easeOut",
                        }}
                    >
                        <h3 className="text-[#132238] text-[24px] font-[600] 795:text-[18px]">{item.title}</h3>
                        ...
                        <div
                            className={`bg-[#A53DFF] transition-all duration-200 ease-in-out absolute z-[1] top-0 left-[-5px] rounded-[12px_0_0_12px] ${
                                hoveredId === item.id ? "w-[5px] min-h-full" : "w-0"
                            }`}
                        ></div>
                        <div
                            className={`bg-[#A53DFF] transition-all duration-200 ease-in-out absolute z-[1] top-0 right-[-5px] rounded-[0_12px_12px_0] ${
                                hoveredId === item.id ? "w-[5px] min-h-full" : "w-0"
                            }`}
                        ></div>
                    </motion.div>
